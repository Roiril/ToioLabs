...
## 9. UIスクリプト等の初期値変更とInspector更新の注意点

### 判断フロー: C#スクリプト側でUIやコンポーネントの初期値を変更した時

```
C#スクリプト内で、通常はInspectorから入力されるようなUI関連のパラメーター（初期テキストや設定値など）を変更した
  │
  ├─ その変更はコード上の初期値（例: `[SerializeField] private string _text = "New";`）を変えただけか？
  │    ├─ YES → 注意！ それだけでは既存のSceneやPrefab上にシリアライズされた値は更新されない ❌
  │    │    │
  │    │    ├─ 解決策 A: `manage_components` 等のMCPツールを用いて、該当コンポーネントのプロパティ（`set_property`）を直接新しい値に更新する。
  │    │    └─ 解決策 B: もし自動変更が難しい場合は、ユーザーに対して「コードの初期値を変更しましたが、Scene/Prefab上には古い値が残っているため、Inspectorから手動で更新してください」と明確に通知する。
  │    │
  │    └─ NO（Awake/Start時などにスクリプトから動的に上書き代入するようにした） → OK ✅
```
