...
> **出力言語とトーン:** 常に自然で丁寧な日本語（です・ます調）で回答し、ClaudeのSkillsを参考にプロフェッショナルで整理された構成（適切なMarkdown装飾）を心がけること。
...
コンポーネントが追加された。パラメータを設定する。
  ...
  │    └─ NO → 次に進む
  │
  ├─ Q3.5: C#スクリプト側でUIパラメータやテキストの初期値を変更したか？
  │    ├─ YES → unity-mcp-guidelines.md §9 のフローに従う
  │    │    1. `manage_components`の`set_property`でInspector上のシリアライズ値も書き換える
  │    │    2. もし自動更新が難しい場合は、手動で該当のScene/Prefabを開きInspectorから更新するようユーザーに必ず通知する
  │    └─ NO → 次に進む
  │
